...
fn dir_response(dirpath: &Path, root_dir: &Path, mut stream: &mut TcpStream) -> io::Result<()> {
    ...
    let mut response: Vec<u8> = if !folder_items.is_empty() {
        ...
        folder_items.iter().map(|path| format!("<a href=\"/{}\">{}</a><br>\r\n", path, path.split('/').last()
            .unwrap())) // turn the path Strings into HTML links; The "/" is important!
...
impl HTTPRequest {
    ...
    fn read_from_tcp_stream(stream: &mut TcpStream) -> io::Result<Self> {
        ...
        return Ok(Self { http_request: String::from_utf8_lossy(&request_buffer).to_string() })
    }
...
impl HTTPResponse {
    ...
    fn new_206_partial_content(content: &Vec<u8>, start_index: &str, end_index: &str) -> Self {
        // cf. https://stackoverflow.com/questions/23071164/grails-ios-specific-returning-video-mp4-file-gives-broken-pipe-exception-g
        let mut http_response: Vec<u8> = format!(
    ...
    fn write_206_partial_file_to_stream(
        filepath: &Path, range: (u64, u64), stream: &mut TcpStream
    ) -> io::Result<()> {
